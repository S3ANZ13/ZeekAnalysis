...
```
  ...
  The quality of the data is very high as the Zeek logs selected for study are military grade. Overall sparsity within the selected log class (conn) is 12.4%.  

```{r Conn Sparsity, echo=FALSE, message=FALSE, warning=FALSE}
library(visdat)
...
```
  Outliers are frequently removed for clustering analyses; however, they will not be removed for this project because the desired outcome in this case relies on there being correlations among the outliers that might identify threat actors’ actions within the monitored network (Mazarbhuiya, 2019).
...
***
    ...
    The report on the data-analysis process includes a description of the technique(s) used. The report includes the calculations performed and their outputs as well as a justification of why the analysis technique(s) were selected and one advantage and one disadvantage of these technique(s). The technique(s) used and the analysis are appropriate for the data.


  Having prepared the data for analysis, a distance (or dissimilarity) matrix will be calculated in order to compare linkage methods. 
...
After selecting the optimum linkage method, the cluster will be cut to various heights informed by an elbow plot, gap statistic plot, and the cluster membership patterns themselves. 

```{r Dendrogram, cache = TRUE }
pltree(hc.complete, cex = 0.6, hang = -1, main = "Dendrogram")
```

```{r Determine Number of Clusters, cache = TRUE, fig.height=3}
library(factoextra)
# Plot cluster results
p1 <- fviz_nbclust(df.conn.scaled, FUN = hcut, method = "wss", 
                   k.max = 10) +
  ggtitle("(A) Elbow method")
p2 <- fviz_nbclust(df.conn.scaled, FUN = hcut, method = "silhouette", 
                   k.max = 10) +
...
```{r Two-Cluster Membership, echo=FALSE, cache=TRUE}
clust.2 <- cutree(hc.average, k = 2)
fviz_cluster(list(data = df.conn.scaled, cluster = clust.2))
```

```{r Four-Cluster Membership, echo=FALSE, cache=TRUE}
clust.4 <- cutree(hc.average, k = 4)
fviz_cluster(list(data = df.conn.scaled, cluster = clust.4))
```
...
Project Outcomes: The project will seek to separate Zeek log data into clusters that will group legitimate traffic types together and isolate anomalous traffic that might indicate a compromise of the network. Support for the alternative hypothesis is found in Mazarbhuiya et. Al (2019), which indicated that this agglomerative hierarchical clustering is an effective way to flag potential network compromise indicators.

```{r Silhouette Score}
hc.final <- hcut(df.conn.scaled, k = 4, hc_method = "average")
fviz_silhouette(hc.final)
```
...
## Analysis Limitation
  ...
  Anomaly detection---while a valuable part of the threat identification process---is not equivalent to threat identification on its own.

## Course of Action Recommendation

  In circumstances where it is not possible or practical to leverage the real-time scanning of a NIDS, 

## Approaches for Further Study

  The results of this study indicate a successful proof-of-concept for security analysis methods where real-time network monitoring is not possible.  However, 

### Additional Feature Combinations

  Despite experimenting with various combinations of variables while conducting this study, the set of tested feature combinations is far from exhaustive.  This study does not claim certainty that an alternative feature selection would not produce more significant clusters.  Additional research should be conducted with varying features from the connectivity logs and even from other log classes.  Particular attention should be focused on increasing the separation of the outliers from legitimate network traffic while keeping the benign 

### Threat Detection from Amalgamated Outliers
